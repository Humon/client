...
public class Tracker_Handler : MonoBehaviour
{
    ...
    private float thetaOffsetX = 180;
    private float thetaOffsetY = -90;
    private float thetaOffsetZ = -180;
    ...
    */
    ...
    void KP_MoveArmToControllerPosition()
    {
        ...
        if (!gripMoving && HoldingDeadTrigger())
        {
            ...
            //cartesianCommandToSend.x = -trackerPosition.x;
            //cartesianCommandToSend.y = -trackerPosition.y;
            //cartesianCommandToSend.z = -trackerPosition.z;
            //cartesianCommandToSend.thetaZ = thetaOffsetX * (pi / 180.0f);// - transform.localRotation.eulerAngles.x * (pi / 180.0f) ;
            //cartesianCommandToSend.thetaY = thetaOffsetY * (pi / 180.0f) + transform.localRotation.eulerAngles.y * (pi / 180.0f);
            //cartesianCommandToSend.thetaX = thetaOffsetZ * (pi / 180.0f);// - transform.localRotation.eulerAngles.z * (pi / 180.0f) ;

            cartesianCommandToSend.x = trackerPosition.z;
            cartesianCommandToSend.y = -trackerPosition.y;
            cartesianCommandToSend.z = -trackerPosition.x;

            cartesianCommandToSend.thetaX = (transform.localRotation.eulerAngles.x + thetaOffsetX) * Mathf.Deg2Rad;
            cartesianCommandToSend.thetaY = (-transform.localRotation.eulerAngles.y + thetaOffsetY) * Mathf.Deg2Rad;
            cartesianCommandToSend.thetaZ = (-transform.localRotation.eulerAngles.z + thetaOffsetZ) * Mathf.Deg2Rad;

            SendCurrentCommand();

            //original values without mathf.deg2rad and offsets
            //cartesianCommandToSend.x = trackerPosition.z;
            //cartesianCommandToSend.y = -trackerPosition.y;
            //cartesianCommandToSend.z = -trackerPosition.x;
